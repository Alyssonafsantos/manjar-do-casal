...
function onTouchStart(event, index){
    ...
    const slideItem = event.currentTarget;
    slideItem.addEventListener('touchmove', onTouchMove, {passive: true});
...
function onTouchEnd(event){
    onMouseUp(event);
    const slideItem = event.currentTarget;
    slideItem.removeEventListener('touchmove', onTouchMove, {passive: true});
...
function initSlider({startAtIndex, autoPlay = true, timeInterval = 0}){
